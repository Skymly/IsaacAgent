...
1. **路径安全**：`read_file` / `write_file` / `list_files` / `diagnose_lua` / `validate_xml` / `parse_log`（相对路径）必须通过 Core `ProjectPathSafety.IsWithinProject` / `Resolve`（或绝对路径 log 白名单）。今日 Tools 仍用内部 `FileToolPathSafety`；切换见 #60（Rag `parse_log` 见 #62）。
...
权威实现：Core [`ProjectPathSafety`](Core.md)（`Resolve` / `IsWithinProject` / Isaac log 白名单）。Tools 内 `FileToolPathSafety` 为过渡副本，行为应与 Core 对齐直至 #60 删除。
