...
| 2026-06-24 | KB-1 | - | **已修复**：从 `vanilla/enums/ModCallbacks.md` + `repentogon/enums/ModCallbacks.md` 完整重建 `ModCallbacks.cs`。修正全部 74 个 vanilla 回调 ID（0-73），新增 `RepentogonCallbacks` 字典（130+ 个 REPENTOGON 专属回调，ID 1000+）和 `RepentogonModifiedIds` 字典（12 个 vanilla 回调的 REPENTOGON 增强 ID）。`GetCallbackInfoTool` 改用 `ModCallbacks.Lookup()` 跨两字典查询并显示 REPENTOGON override ID。`DiagnoseLuaTool` 未知回调检查扩展到 REPENTOGON 字典。`ApiDocChunker` 新增 REPENTOGON 回调 chunk。`QuickReferenceViewModel` 列表包含 REPENTOGON 回调。+5 新测试（ID 正确性、REPENTOGON 完整性、ID 范围、override 映射、Lookup 跨字典），149 全过 |
| 2026-06-24 | #8 | - | `AgentSession.TrimHistory` 只按消息条数（≤50）裁剪，单个 `list_files` 结果几万字符只算 1 条 → 新增 `MaxContextChars = 120_000`（~30k token）字符预算，条数 **或** 字符超限即裁剪；字符裁剪阶段复用 tool_call/tool_result 孤儿保护规则；+2 测试，151 全过 |
| 2026-06-24 | #10 | - | `OpenAICompatibleProvider` / `OllamaProvider` 流式读取无 idle 超时，服务器卡住时 UI 无限"生成中" → 用 `CancellationTokenSource.CreateLinkedTokenSource` + `CancelAfter(90s)` 每次 `ReadLineAsync` 后重置；卡住抛 `TimeoutException`；附带修复 `while(!reader.EndOfStream)` 的同步 `Read()` 问题；+4 测试，155 全过 |
| 2026-06-24 | #11 | - | `PrewarmRagIndexAsync` 失败只 log warning，用户无感知 → 失败时调用 `SettingsViewModel.SetIndexStatus` 显示错误信息 |
| 2026-06-24 | P3-1 | - | **已修复**：CI workflow 从单一 windows-latest 扩展为三 job 矩阵：`ci-lib`（ubuntu-latest + macos-latest，Nuke `CiLib` 目标仅构建库项目 + 测试）、`ci-windows`（windows-latest，Nuke `CiAll` 含 format + 全解决方案）、`release`（仅 `v*` tag，`dotnet publish` self-contained win-x64 单文件 + GitHub Release draft）。Nuke 新增 `UnitTestLib` / `CiLib` 目标。App csproj 在非 Windows 抑制 CA1416 以支持跨平台测试构建。本地 `CiLib` + `CiAll` 均通过，155 测试全过 |
